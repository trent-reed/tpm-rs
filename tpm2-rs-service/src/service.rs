...
impl<'a, Deps: ServiceDeps> Service<'a, Deps> {
    ...
    fn fill_error(&mut self, response: &mut Deps::Response, error: TpmError) -> usize {
        ...
        if response.write(6, &error.as_u32().to_be_bytes()).is_err() {
            return 0;
        }
        10
    }
    ...
    fn execute_command(&mut self, buffers: impl TpmBuffers) -> Result<usize, TpmError> {
        ...
        let _session = request.read_be_u16().ok_or(TPM_RC_COMMAND_SIZE)?;
        let size = request.read_be_u32().ok_or(TPM_RC_COMMAND_SIZE)?;
        if size as usize != request_size {
            return Err(TPM_RC_COMMAND_SIZE.into());
        }
        let command_code = request.read_be_u32().ok_or(TPM_RC_COMMAND_SIZE)?;
        ...
        match Command(command_code) {
            ...
            _ => Err(TPM_RC_COMMAND_CODE.into()),
        }?;
